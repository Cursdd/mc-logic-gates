...
$execute unless score $uiSize lg.VAR matches 26.. run summon interaction ~$(uiOffset) ~ ~0.5 {Tags:["lg.uiHitbox"],width:$(uiSize),height:$(uiSize)}

#If the ui size > 25, summon the appropiate amount of hitboxes
$execute if score $uiSize lg.VAR matches 26.. positioned ~ ~ ~-$(uiHitboxOffset) run function lg:spawn_ui/3/for_hitbox/init