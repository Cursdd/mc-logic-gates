...
$execute unless score $uiSize lg.VAR matches 26.. positioned ~ ~ ~1 run summon interaction ~0.5 ~ ~-$(uiOffset) {Tags:["lg.uiHitbox"],width:$(uiSize),height:$(uiSize)}

#If the ui size > 25, summon the appropiate amount of hitboxes
$execute if score $uiSize lg.VAR matches 26.. positioned ~-$(uiHitboxOffset) ~ ~1 run function lg:spawn_ui/2/for_hitbox/init